...
RESOLVED_ROUTES_FILE="/data/data/com.termux/files/usr/tmp/.st_resolved_routes"
...
# Optional $1: timeout seconds (default 12)
# Optional $2: branch name to verify existence (e.g. "release")
fn_measure_git_candidates() {
    ...
    while IFS='|' read -r name host is_official git_url; do
        (
            ...
            start="$(date +%s.%N)"
            if [[ -n "$branch" ]]; then
                output="$(timeout "${timeout_seconds}s" git ls-remote --heads "$git_url" "$branch" 2>/dev/null)"
                if [[ -n "$output" ]]; then
                    end="$(date +%s.%N)"
...
# Also writes all successful candidates to RESOLVED_ROUTES_FILE (one per line: host|git_url)
# Optional $3: branch name to verify during speed test (e.g. "release")
fn_resolve_download_route() {
    local operation_name="$1"
    local git_url="$2"
    local branch="${3:-}"

    : > "$RESOLVED_ROUTES_FILE" 2>/dev/null || true
    ...
    if [[ "$google_status" -eq 0 ]]; then
        ...
        if fn_read_yes_no_prompt "是否使用 GitHub 官方线路（推荐）" true "回车=是；输入 n=测速镜像并手动选择。"; then
            ...
            echo "github.com|${git_url}" > "$RESOLVED_ROUTES_FILE"
            return 0
        fi
    else
        [[ -z "$google_failed" ]] && google_failed="未知"
    ...
    mapfile -t all_candidates < <(fn_get_github_git_candidates "$git_url")
    ...
    local branch_hint=""
    [[ -n "$branch" ]] && branch_hint=" (验证分支: ${branch})"
    fn_print_warning "正在并行测速 ${#all_candidates[@]} 条线路${branch_hint}..."
    local measured
    mapfile -t measured < <(printf '%s\n' "${all_candidates[@]}" | fn_measure_git_candidates 12 "$branch")
    ...
    : > "$RESOLVED_ROUTES_FILE"
    local s_line s_elapsed s_name s_host s_is_official s_url
    for s_line in "${successful[@]}"; do
        IFS='|' read -r s_elapsed s_name s_host s_is_official s_url <<<"$s_line"
        echo "${s_host}|${s_url}" >> "$RESOLVED_ROUTES_FILE"
    done
...
fn_start_st() {
    ...
    # 检测当前是否处于 Luker 模式
    local current_mode
    current_mode="$(fn_get_st_current_mode)"
    if [[ "$current_mode" == "luker" ]]; then
        fn_print_warning "提示：当前酒馆目录正处于【Luker 模式】分支！"
        echo -e "${CYAN}您当前启动的是 Luker。如需切换回原版酒馆，请使用主菜单 [14] 切换模式。${NC}\n"
        if ! fn_read_yes_no_prompt "是否继续启动 Luker" true ""; then
            fn_print_warning "已取消启动。"
            fn_press_any_key
            return
        fi
    fi
    ...
    npm config set registry https://registry.npmmirror.com
    echo -e "${YELLOW}环境准备就绪，正在启动服务...${NC}"
...
fn_install_st() {
    ...
    if [ -f "$ST_DIR/start.sh" ]; then
        ...
        if ! selected_route="$(fn_resolve_download_route "下载酒馆主程序" "https://github.com/SillyTavern/SillyTavern.git" "$REPO_BRANCH")"; then
        ...
        if ! fn_run_git_with_progress "下载酒馆主程序" false git clone --progress --depth 1 -b "$REPO_BRANCH" "$route_url" "$ST_DIR"; then
            fn_print_warning "线路 [${route_host}] 下载失败，正在尝试其他可用线路..."
            rm -rf "$ST_DIR"

            local clone_success=false
            local alt_host alt_url
            while IFS='|' read -r alt_host alt_url; do
                [[ -z "$alt_host" || "$alt_host" == "$route_host" ]] && continue
                fn_print_warning "正在尝试线路 [${alt_host}]..."
                if fn_run_git_with_progress "下载酒馆主程序" false git clone --progress --depth 1 -b "$REPO_BRANCH" "$alt_url" "$ST_DIR"; then
                    clone_success=true
                    break
                fi
                rm -rf "$ST_DIR"
            done <"$RESOLVED_ROUTES_FILE"

            if ! $clone_success; then
                if fn_git_last_log_contains_regex "Failed to connect to|Could not connect to server|Connection timed out|Could not resolve host"; then
                    fn_write_git_network_troubleshooting
                fi
                fn_print_error "所有线路均下载失败！Git输出: $(fn_git_last_log_tail 2)"
                fn_press_any_key
                return
            fi
        fi
...
fn_update_st() {
    ...
    local selected_route
    if ! selected_route="$(fn_resolve_download_route "更新酒馆" "https://github.com/SillyTavern/SillyTavern.git" "$REPO_BRANCH")"; then
...
fn_rollback_st() {
    ...
    local selected_route
    if ! selected_route="$(fn_resolve_download_route "回退前获取版本信息" "https://github.com/SillyTavern/SillyTavern.git" "$REPO_BRANCH")"; then
...
fn_get_st_current_mode() {
    if [ ! -d "$ST_DIR/.git" ]; then
        if [ -f "$ST_DIR/package.json" ]; then
            if grep -qi "luker" "$ST_DIR/package.json" 2>/dev/null || grep -qi "funnycups" "$ST_DIR/package.json" 2>/dev/null; then
                echo "luker"
                return
            fi
        fi
    ...
    branch="$(git -C "$ST_DIR" symbolic-ref --short -q HEAD 2>/dev/null || git -C "$ST_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || true)"
    if [[ "$branch" == "luker" || "$branch" == *"luker"* ]]; then
        echo "luker"
        return
    fi
    remote_url="$(git -C "$ST_DIR" config --get "branch.${branch}.remote" 2>/dev/null || true)"
    if [[ "$remote_url" == "luker" ]]; then
        echo "luker"
        return
    fi
    if [ -f "$ST_DIR/package.json" ]; then
...
fn_run_luker_npm_install() {
    if [ ! -d "$LUKER_DIR" ]; then return 1; fi
    cd "$LUKER_DIR" || return 1

    fn_print_warning "正在配置 NPM 镜像加速并安装依赖包..."
    npm config set registry https://registry.npmmirror.com

    if npm install --no-audit --no-fund; then
        fn_print_success "Luker 依赖包安装完成。"
        return 0
    fi

    fn_print_warning "常规安装失败，尝试使用 --legacy-peer-deps 重试..."
    if npm install --legacy-peer-deps --no-audit --no-fund; then
        fn_print_success "Luker 依赖包安装完成。"
        return 0
    fi

    fn_print_warning "国内镜像安装失败，切换至 NPM 官方源进行尝试..."
    npm config delete registry
    local exit_code
    ...
    if [ $exit_code -eq 0 ]; then
        fn_print_success "使用官方源安装 Luker 依赖成功！"
        return 0
    else
        fn_print_error "Luker 依赖安装失败，请检查网络或日志。"
        return 1
    fi
}
...
fn_start_luker() {
    clear
    fn_print_header "启动 Luker"
    
    local target_run_dir=""
    local is_branch_mode=false

    if [ -d "$LUKER_DIR" ]; then
        target_run_dir="$LUKER_DIR"
    elif [ -d "$ST_DIR" ]; then
        local current_mode
        current_mode="$(fn_get_st_current_mode)"
        if [[ "$current_mode" == "luker" ]]; then
            target_run_dir="$ST_DIR"
            is_branch_mode=true
        else
            fn_print_warning "检测到当前酒馆目录处于【原版酒馆模式】，未处于 Luker 分支！"
            echo -e "${CYAN}如需在酒馆目录中运行 Luker，请先使用 [14] 切换为 Luker 分支；${NC}"
            echo -e "${CYAN}或者使用 [13] 进行 Luker 独立首次部署。${NC}\n"
            if ! fn_read_yes_no_prompt "是否仍尝试在当前目录启动" false ""; then
                fn_print_warning "已取消启动。"
                fn_press_any_key
                return 1
            fi
        ...
        fn_print_warning "未检测到 Luker 或酒馆安装目录，请先进行部署。"
        fn_press_any_key
        return 1
    fi

    cd "$target_run_dir" || { fn_print_error "无法进入目录: $target_run_dir"; fn_press_any_key; return 1; }

    echo -e "正在配置 NPM 镜像并准备启动环境..."
    npm config set registry https://registry.npmmirror.com
    echo -e "${YELLOW}环境准备就绪，正在启动 Luker 服务...${NC}"
    echo -e "${CYAN}如果提示服务已启动，可在手机浏览器中访问对应端口（如 http://127.0.0.1:3000 或 8000）${NC}\n"
    ...
    echo -e "\n${YELLOW}Luker 已停止运行。${NC}"
    fn_press_any_key
}

fn_switch_st_luker_mode() {
    clear
    fn_print_header "原版酒馆 ↔ Luker 模式无缝切换"
    if [ ! -d "$ST_DIR/.git" ]; then
        fn_print_error "酒馆目录不存在或不是有效的 Git 仓库！"
        fn_press_any_key
        return 1
    fi
    ...
    current_branch="$(git symbolic-ref --short -q HEAD 2>/dev/null || git rev-parse --short HEAD 2>/dev/null || echo "未知")"

    echo -e "      当前检测状态: $([ "$current_mode" == "luker" ] && echo "${GREEN}【Luker 模式】${NC}" || echo "${CYAN}【原版酒馆模式】${NC}")"
    echo -e "      当前所在分支: ${YELLOW}${current_branch}${NC}"
    echo -e "      酒馆目录路径: ${CYAN}${ST_DIR}${NC}\n"
    echo -e "${YELLOW}[重要提示] 模式切换前建议提前做好数据备份！${NC}\n"

    if [[ "$current_mode" == "luker" ]]; then
        echo -e "      [1] ${CYAN}回退切换至【原版酒馆 (SillyTavern)】${NC}"
        echo -e "      [2] ${GREEN}刷新拉取 Luker 最新代码并重装依赖${NC}"
        echo -e "      [0] 返回上一级\n"
        local choice
        choice="$(fn_read_menu_prompt "0-2")"
        case "$choice" in
            1)
                ...
                echo -e "正在配置 NPM 镜像并校验安装依赖..."
                npm config set registry https://registry.npmmirror.com
                npm install --no-audit --no-fund || npm install --legacy-peer-deps --no-audit --no-fund
                fn_print_success "已成功切换回原版酒馆模式！"
                fn_press_any_key
                ;;
            2)
                fn_print_warning "正在拉取 Luker 最新代码..."
                git fetch --depth=1 luker release:refs/remotes/luker/release 2>/dev/null || git fetch luker 2>/dev/null || true
                if git rev-parse --verify refs/remotes/luker/release >/dev/null 2>&1; then
                    git reset --hard refs/remotes/luker/release
                else
                    git pull luker release 2>/dev/null || git pull
                fi
                echo -e "正在更新依赖..."
                npm config set registry https://registry.npmmirror.com
                npm install --no-audit --no-fund || npm install --legacy-peer-deps --no-audit --no-fund
                fn_print_success "Luker 分支更新完毕！"
                fn_press_any_key
                ;;
            0) return ;;
        esac
    else
        echo -e "      [1] ${GREEN}切换至【Luker (funnycups/Luker)】${NC}"
        echo -e "      [0] 返回上一级\n"
        local choice
        choice="$(fn_read_menu_prompt "0-1")"
        case "$choice" in
            1)
                fn_print_warning "正在配置 Luker 远程仓库源..."
                local luker_repo_url="https://github.com/funnycups/Luker.git"
                local luker_mirror_url="https://ghfast.top/https://github.com/funnycups/Luker.git"
                if ! git remote | grep -q "^luker$"; then
                    git remote add luker "$luker_mirror_url" 2>/dev/null || git remote set-url luker "$luker_mirror_url"
                else
                    git remote set-url luker "$luker_mirror_url" 2>/dev/null || true
                fi
                git config remote.luker.fetch "+refs/heads/*:refs/remotes/luker/*" 2>/dev/null || true

                fn_print_warning "正在获取 Luker 远程分支信息 (git fetch luker)..."
                if ! git fetch --depth=1 luker release:refs/remotes/luker/release 2>/dev/null; then
                    if ! git fetch --depth=1 "$luker_repo_url" release:refs/remotes/luker/release 2>/dev/null; then
                        fn_run_git_with_progress "获取 Luker 分支" false git fetch luker || fn_print_warning "fetch 异常，尝试强制切换..."
                    fi
                fi

                fn_print_warning "正在切换至 luker 分支..."
                if git rev-parse --verify refs/remotes/luker/release >/dev/null 2>&1; then
                    git checkout -B luker refs/remotes/luker/release
                elif git rev-parse --verify refs/remotes/luker/main >/dev/null 2>&1; then
                    git checkout -B luker refs/remotes/luker/main
                elif git show-ref --verify --quiet refs/heads/luker; then
                    git switch luker || git checkout luker
                else
                    git checkout -b luker
                fi

                echo -e "正在配置 NPM 镜像并安装 Luker 依赖..."
                npm config set registry https://registry.npmmirror.com
                if npm install --no-audit --no-fund; then
                    fn_print_success "Luker 依赖安装完成。"
                else
                    npm install --legacy-peer-deps --no-audit --no-fund || fn_print_warning "依赖安装遇到警告，可直接尝试启动。"
                fi

                if grep -q '"build"' package.json 2>/dev/null; then
                    fn_print_warning "正在执行构建 npm run build..."
                    npm run build || true
                fi

                fn_print_success "已成功切换至 Luker 模式！"
                fn_press_any_key
                ;;
            0) return ;;
        esac
...
fn_install_luker() {
    clear
    fn_print_header "Luker 首次部署向导"
    echo -e "${CYAN}项目地址: https://github.com/funnycups/Luker.git${NC}"
    echo -e "正在检查本地运行环境..."

    # 1. 检查 Node.js / Git 基础环境是否已就绪
    local need_install_deps=false
    if ! fn_check_command "node" || ! fn_check_command "git"; then
        need_install_deps=true
    fi

    if $need_install_deps; then
        fn_print_warning "检测到缺少 Node.js 或 Git 基础依赖，正在配置并安装..."
        while true; do
            if ! fn_update_termux_source; then
                if ! fn_read_yes_no_prompt "软件源配置失败，是否重试" true ""; then
                    fn_print_error_exit "用户取消操作。"
        ...
        fn_run_termux_apt_noninteractive upgrade || fn_print_error_exit "系统包升级失败！"
        fn_run_termux_apt_noninteractive install git nodejs-lts rsync zip unzip || fn_print_error_exit "基础依赖安装失败！"
        fn_print_success "基础依赖已安装完毕。"
    else
        fn_print_success "检测到 Node.js 与 Git 基础环境已就绪，跳过基础安装直接部署 Luker！"
    fi

    # 2. 下载或克隆 Luker 源码
    fn_print_header "下载 Luker 主程序"
    if [ -d "$LUKER_DIR/.git" ]; then
        fn_print_warning "检测到 Luker 目录已存在，跳过克隆。"
    elif [ -d "$LUKER_DIR" ] && [ -n "$(ls -A "$LUKER_DIR")" ]; then
        fn_print_warning "目录 $LUKER_DIR 已存在但非 Git 仓库，正在准备更新/覆盖..."
    else
        local selected_route
        if ! selected_route="$(fn_resolve_download_route "下载 Luker 主程序" "https://github.com/funnycups/Luker.git")"; then
            fn_print_error "未能选定可用下载线路，尝试使用直连/默认源..."
            selected_route="github.com|https://github.com/funnycups/Luker.git"
        fi

        local route_host route_url
        IFS='|' read -r route_host route_url <<<"$selected_route"

        fn_print_warning "正在使用线路 [${route_host}] 下载 Luker..."
        if ! fn_run_git_with_progress "下载 Luker 主程序" false git clone --progress --depth 1 "$route_url" "$LUKER_DIR"; then
            fn_print_warning "线路 [${route_host}] 下载失败，正在尝试备用线路..."
            rm -rf "$LUKER_DIR"

            local clone_success=false
            if [ -f "$RESOLVED_ROUTES_FILE" ]; then
                local alt_host alt_url
                while IFS='|' read -r alt_host alt_url; do
                    [[ -z "$alt_host" || "$alt_host" == "$route_host" ]] && continue
                    fn_print_warning "正在尝试线路 [${alt_host}]..."
                    if fn_run_git_with_progress "下载 Luker 主程序" false git clone --progress --depth 1 "$alt_url" "$LUKER_DIR"; then
                        clone_success=true
                        break
                    fi
                    rm -rf "$LUKER_DIR"
            ...
            if ! $clone_success; then
                fn_print_error "所有自动线路克隆均失败，正在尝试备用镜像加速代理..."
                if ! fn_run_git_with_progress "备用镜像克隆" false git clone --progress --depth 1 "https://ghfast.top/https://github.com/funnycups/Luker.git" "$LUKER_DIR"; then
                    fn_print_error "Luker 源码下载失败，请检查网络！"
                    fn_press_any_key
                    return 1
                fi
            fi
        fi
        fn_print_success "Luker 源码下载完成。"
    fi

    # 3. 安装依赖与构建
    fn_print_header "配置并安装 Luker 依赖"
    if ! fn_run_luker_npm_install; then
        fn_print_error "Luker 依赖安装失败，部署中断。"
        fn_press_any_key
        return 1
    fi
    ...
    cd "$LUKER_DIR" || return 1
    if grep -q '"build"' package.json 2>/dev/null; then
        fn_print_warning "检测到构建命令，正在执行 npm run build..."
        npm run build || fn_print_warning "构建未完全成功，将继续尝试运行..."
    fi

    echo -e "\n${GREEN}${BOLD}Luker 首次部署完成！即将启动服务...${NC}"
    sleep 2
    fn_start_luker
}
...
fn_update_luker() {
    clear
    fn_print_header "更新 Luker"
    if [ ! -d "$LUKER_DIR/.git" ]; then
        fn_print_warning "未找到 Luker Git 仓库，请先进行完整部署。"
        fn_press_any_key
        return
    fi
    cd "$LUKER_DIR" || return

    fn_print_warning "正在拉取最新代码..."
    if fn_run_git_with_progress "更新 Luker 代码" false git pull --progress; then
        fn_print_success "代码拉取成功，正在更新依赖..."
        fn_run_luker_npm_install
        if grep -q '"build"' package.json 2>/dev/null; then
            npm run build
        fi
        fn_print_success "Luker 更新完成！"
    else
        fn_print_error "Luker 更新失败，请检查网络。"
    fi
    fn_press_any_key
}
...
fn_uninstall_luker() {
    clear
    fn_print_header "卸载 Luker"
    if [ ! -d "$LUKER_DIR" ]; then
        fn_print_warning "未检测到 Luker 安装目录。"
        fn_press_any_key
        return
    fi

    if fn_read_yes_no_prompt "确认要卸载 Luker 吗？(这将删除 $LUKER_DIR)" false ""; then
        rm -rf "$LUKER_DIR"
        fn_print_success "Luker 已成功卸载。"
    else
        fn_print_warning "操作已取消。"
    fi
    fn_press_any_key
...
fn_menu_luker_manage() {
    while true; do
        clear
        fn_print_header "Luker 管理"
        local status_text="${RED}未安装${NC}"
        if [ -d "$LUKER_DIR" ]; then
            status_text="${GREEN}已安装 (独立目录)${NC}"
        elif [ -d "$ST_DIR" ]; then
            local current_st_mode
            current_st_mode="$(fn_get_st_current_mode)"
            if [[ "$current_st_mode" == "luker" ]]; then
                status_text="${GREEN}已切换至酒馆 Luker 分支${NC}"
            fi
        fi
        echo -e "      当前状态: ${status_text}"
        echo -e "      独立安装路径: ${CYAN}${LUKER_DIR}${NC}\n"

        echo -e "      [1] ${GREEN}启动 Luker${NC}"
        echo -e "      [2] ${CYAN}原版酒馆 ↔ Luker 模式切换${NC}"
        if [ -d "$LUKER_DIR" ]; then
            echo -e "      [3] ${CYAN}更新 Luker (独立版)${NC}"
            echo -e "      [4] ${YELLOW}重新安装依赖 (npm install)${NC}"
            echo -e "      [5] ${RED}卸载 Luker (独立版)${NC}"
        else
            echo -e "      [3] ${GREEN}首次部署 Luker 独立版 (funnycups/Luker)${NC}"
        fi
        echo -e "      [0] ${CYAN}返回上一级${NC}\n"
        ...
        local choice
        choice="$(fn_read_menu_prompt "$allowed")"
        case "$choice" in
            1) fn_start_luker ;;
            2) fn_switch_st_luker_mode ;;
            3)
                if [ -d "$LUKER_DIR" ]; then
                    fn_update_luker
                else
...
fn_menu_lab() {
    while true; do
        ...
        echo -e "      [02] ${CYAN}Luker (funnycups/Luker)${NC}"
        echo -e "      [03] ${CYAN}酒馆配置管理${NC}"
        echo -e "      [00] ${CYAN}返回主菜单${NC}\n"
        local choice
        choice="$(fn_read_menu_prompt "0-3")"
        case $choice in
            1) fn_menu_gcli_manage ;;
            2) fn_menu_luker_manage ;;
            3) fn_menu_st_config ;;
            0) break ;;
            *) fn_print_error "输入无效，请按提示重试。"; sleep 1 ;;
        esac
...
fn_ensure_termux_storage() {
    ...
    if command -v termux-setup-storage >/dev/null 2>&1; then
        fn_print_warning "未检测到 Termux 存储访问权限，正在请求权限..."
        fn_print_info "如果手机弹出存储权限申请窗口，请点击【允许】。"
        termux-setup-storage
        local wait_sec=0
        while [ $wait_sec -lt 10 ]; do
            if [ -d "$HOME/storage/shared" ] && [ -w "$HOME/storage/shared" ]; then
                fn_print_success "存储权限已就绪！"
                return 0
            fi
            sleep 1
            wait_sec=$((wait_sec + 1))
    ...
    fn_print_warning "未能自动获取共享存储权限，备份文件可能会暂存到 Termux 本地目录。"
    return 1
}

fn_get_backup_target_dir() {
    local candidate
    for candidate in "$HOME/storage/shared/Download" "$HOME/storage/shared" "/sdcard/Download" "/sdcard" "$HOME"; do
...
fn_full_system_backup() {
    clear
    fn_print_header "Termux 全量环境与数据大打包"
    echo -e "  ${YELLOW}说明：此功能将完整打包当前 Termux 环境下的全部数据：${NC}"
    echo -e "    - 所有系统组件 (Node.js / Python / Git / 依赖包)"
    echo -e "    - 全部酒馆目录、Luker、扩展插件与配置"
    echo -e "    - 用户角色卡、聊天记录与所有环境设置"
    echo -e "  ${CYAN}适合整机迁移、换手机时一键克隆使用。${NC}\n"

    fn_ensure_termux_storage

    if ! fn_read_yes_no_prompt "确认开始执行 Termux 全量打包吗？" true ""; then
        fn_print_warning "操作已取消。"
        fn_press_any_key
        return
    fi
    ...
    local archive_name="termux_backup_${timestamp}.tar.gz"
    local target_dir
    target_dir="$(fn_get_backup_target_dir)"
    local target_file="${target_dir}/${archive_name}"

    fn_print_warning "正在全量压缩 Termux 环境，这可能需要 1~3 分钟，请稍候..."
    echo -e "  目标输出位置: ${CYAN}${target_file}${NC}"

    if tar -czf "$target_file" \
        --exclude="./storage" \
        --exclude="./usr/tmp/*" \
        --exclude="./.cache/*" \
        --exclude="*.tar.gz" \
        --exclude="*.zip" \
        -C /data/data/com.termux/files . 2>/dev/null; then
        ...
        fn_print_success "全量打包成功！"
        echo -e "  ----------------------------------------------------"
        echo -e "  ${GREEN}✔ 文件名称: ${archive_name}${NC}"
        echo -e "  ${GREEN}✔ 文件大小: ${file_size}${NC}"
        echo -e "  ${GREEN}✔ 保存路径: ${target_file}${NC}"
        echo -e "  ----------------------------------------------------"
        echo -e "  ${YELLOW}提示：可在手机文件管理器（如 MT管理器/系统下载）的 Download 文件夹中找到。${NC}"
    else
        fn_print_error "全量打包失败，请检查剩余磁盘空间与存储权限！"
    fi
    fn_press_any_key
}

fn_precision_tavern_backup() {
    clear
    fn_print_header "酒馆资产轻量精准打包"
    echo -e "  ${YELLOW}说明：仅提取酒馆核心资产并剔除 node_modules 等冗余文件：${NC}"
    echo -e "    - 用户数据 (data/ 包含角色卡、聊天记录、世界书、预设)"
    echo -e "    - 系统配置 (config.yaml / config/)"
    echo -e "    - 服务端插件 (plugins/)"
    echo -e "    - 前端第三方扩展 (public/scripts/extensions/)"
    echo -e "  ${CYAN}体积小（通常仅数十兆），非常适合日常备份与版本迁移。${NC}\n"

    # 扫描候选酒馆目录
    local candidates=()
    [ -d "$ST_DIR" ] && candidates+=("$ST_DIR")
    [ -d "$LUKER_DIR" ] && candidates+=("$LUKER_DIR")
    [ -d "$PWD" ] && [ "$PWD" != "$ST_DIR" ] && [ "$PWD" != "$LUKER_DIR" ] && [ -f "$PWD/package.json" ] && candidates+=("$PWD")
    [ -d "$HOME/SillyTavern" ] && [ "$HOME/SillyTavern" != "$ST_DIR" ] && candidates+=("$HOME/SillyTavern")

    if [ ${#candidates[@]} -eq 0 ]; then
        fn_print_error "未检测到已安装的酒馆或 Luker 目录！"
        fn_press_any_key
        return
    fi

    local selected_dir="${candidates[0]}"
    if [ ${#candidates[@]} -gt 1 ]; then
        echo -e "  检测到多个酒馆实例，请选择要打包的目标："
        local i=1
        ...
        echo ""
        local sel
        sel="$(fn_read_menu_prompt "1-${#candidates[@]}")"
        selected_dir="${candidates[$((sel - 1))]}"
    fi

    echo -e "  当前选定打包目录: ${CYAN}${selected_dir}${NC}\n"

    # 确保 zip 命令可用
    if ! command -v zip >/dev/null 2>&1; then
        fn_print_warning "正在安装 zip 打包工具..."
    ...
    local timestamp
    timestamp="$(date +%Y%m%d_%H%M%S)"
    local archive_name="sillytavern_data_backup_${timestamp}.zip"
    local target_dir
    target_dir="$(fn_get_backup_target_dir)"
    local target_file="${target_dir}/${archive_name}"
    ...
    # 临时暂存区
    local temp_root
    temp_root="$(mktemp -d 2>/dev/null || mktemp -d -t 'st-backup.XXXXXX')"
    local stage_root="$temp_root/payload"
    mkdir -p "$stage_root/config" "$stage_root/data" "$stage_root/plugins" "$stage_root/public"

    fn_print_warning "正在提取核心数据与配置..."

    # 1. 拷贝配置
    if [ -f "$selected_dir/config.yaml" ]; then
        cp -a "$selected_dir/config.yaml" "$stage_root/config/config.yaml"
    fi
    if [ -d "$selected_dir/config" ]; then
        cp -a "$selected_dir/config/." "$stage_root/config/"
    fi

    # 2. 拷贝数据
    if [ -d "$selected_dir/data" ]; then
        cp -a "$selected_dir/data/." "$stage_root/data/"
    fi

    # 3. 拷贝插件
    if [ -d "$selected_dir/plugins" ]; then
        cp -a "$selected_dir/plugins/." "$stage_root/plugins/"
    fi

    # 4. 拷贝扩展
    if [ -d "$selected_dir/public/scripts/extensions" ]; then
        mkdir -p "$stage_root/public/scripts/extensions"
        cp -a "$selected_dir/public/scripts/extensions/." "$stage_root/public/scripts/extensions/"
    elif [ -d "$selected_dir/extensions" ]; then
        mkdir -p "$stage_root/public/scripts/extensions/third-party"
        cp -a "$selected_dir/extensions/." "$stage_root/public/scripts/extensions/third-party/"
    fi

    # 统计数据
    local char_count=0
    local chat_count=0
    if [ -d "$stage_root/data" ]; then
        char_count="$(find "$stage_root/data" -type f -name "*.png" 2>/dev/null | wc -l | tr -d ' ')"
        chat_count="$(find "$stage_root/data" -type f -name "*.jsonl" 2>/dev/null | wc -l | tr -d ' ')"
    fi

    fn_print_warning "正在压缩生成 ZIP 备份包 (角色卡: ${char_count} 张, 聊天记录: ${chat_count} 份)..."
    (
        cd "$stage_root" || exit 1
        zip -qr "$target_file" config data plugins public
    )
    local zip_status=$?

    rm -rf "$temp_root"

    if [ $zip_status -eq 0 ] && [ -f "$target_file" ]; then
        local file_size="未知"
        if command -v ls >/dev/null 2>&1; then
            file_size="$(ls -lh "$target_file" | awk '{print $5}')"
        fi
        fn_print_success "精准数据打包成功！"
        echo -e "  ----------------------------------------------------"
        echo -e "  ${GREEN}✔ 文件名称: ${archive_name}${NC}"
        echo -e "  ${GREEN}✔ 文件大小: ${file_size}${NC}"
        echo -e "  ${GREEN}✔ 资产统计: 角色卡 ${char_count} 张 | 聊天记录 ${chat_count} 条${NC}"
        echo -e "  ${GREEN}✔ 保存路径: ${target_file}${NC}"
        echo -e "  ----------------------------------------------------"
    else
        fn_print_error "打包压缩失败，请检查存储权限与空间！"
    fi
    fn_press_any_key
}
...
fn_restore_backup() {
    clear
    fn_print_header "解压与恢复备份包"
    fn_ensure_termux_storage

    # 自动搜索可能的备份文件
    local search_dirs=("$HOME/storage/shared/Download" "$HOME/storage/shared" "/sdcard/Download" "$HOME")
    local found_files=()
    for sdir in "${search_dirs[@]}"; do
        if [ -d "$sdir" ]; then
            while IFS= read -r f; do
                [ -n "$f" ] && found_files+=("$f")
            done < <(find "$sdir" -maxdepth 2 \( -name "termux_backup_*.tar.gz" -o -name "sillytavern_*.zip" -o -name "*.tar.gz" -o -name "*.zip" \) 2>/dev/null | head -n 15)
        fi
    done
    ...
    if [ ${#found_files[@]} -gt 0 ]; then
        ...
        for item in "${found_files[@]}"; do
            ...
            fname="$(basename "$item")"
            local fsize
            fsize="$(ls -lh "$item" 2>/dev/null | awk '{print $5}')"
            echo -e "    [$idx] ${CYAN}${fname}${NC} (${fsize}) -> ${DIM}${item}${NC}"
            idx=$((idx + 1))
        done
        echo -e "    [0] 手动输入文件绝对路径\n"

        local pick
        pick="$(fn_read_menu_prompt "0-${#found_files[@]}")"
        if [ "$pick" -eq 0 ]; then
            echo -n "  请输入备份文件的绝对路径: "
            read -r selected_file
        else
            selected_file="${found_files[$((pick - 1))]}"
        fi
    else
        echo -n "  未自动搜寻到备份包，请输入备份文件的完整绝对路径: "
        read -r selected_file
    fi

    if [ -z "$selected_file" ] || [ ! -f "$selected_file" ]; then
        fn_print_error "文件不存在或路径无效：$selected_file"
        fn_press_any_key
        return
    fi

    echo -e "\n  选定文件: ${CYAN}${selected_file}${NC}"

    # 判断是 tar.gz 还是 zip
    if [[ "$selected_file" == *.tar.gz ]] || [[ "$selected_file" == *.tgz ]]; then
        echo -e "\n  ${YELLOW}[检测为 Termux 全量环境包]${NC}"
        echo -e "  默认将解压覆盖至 Termux 根目录: ${CYAN}/data/data/com.termux/files${NC}"
        echo -e "  也可以输入自定义目标解压目录。"
        echo -n "  请输入目标解压路径 [直接回车默认 /data/data/com.termux/files]: "
        read -r dest_dir
        [ -z "$dest_dir" ] && dest_dir="/data/data/com.termux/files"

        if ! fn_read_yes_no_prompt "确认将备份解压还原到 $dest_dir 吗？" true ""; then
            fn_print_warning "已取消恢复操作。"
            fn_press_any_key
            return
        fi

        mkdir -p "$dest_dir"
        fn_print_warning "正在解压并还原全量环境，请稍候..."
        if tar -xzf "$selected_file" -C "$dest_dir"; then
            fn_print_success "全量环境还原完成！"
            echo -e "  ${GREEN}提示：建议退出 Termux 并重新打开，以加载全部环境配置。${NC}"
        else
            fn_print_error "解压还原失败！请检查文件完整性。"
        fi

    elif [[ "$selected_file" == *.zip ]]; then
        echo -e "\n  ${YELLOW}[检测为酒馆精准数据包]${NC}"
        local default_dest="$ST_DIR"
        [ ! -d "$default_dest" ] && [ -d "$LUKER_DIR" ] && default_dest="$LUKER_DIR"
        [ ! -d "$default_dest" ] && default_dest="$PWD"

        echo -e "  请输入要还原的目标酒馆目录路径："
        echo -e "  [直接回车默认使用: ${CYAN}${default_dest}${NC}]"
        echo -n "  目标路径: "
        read -r dest_dir
        [ -z "$dest_dir" ] && dest_dir="$default_dest"

        if ! fn_read_yes_no_prompt "确认将数据恢复到 $dest_dir 吗？" true ""; then
            fn_print_warning "已取消恢复操作。"
            fn_press_any_key
            return
        fi
        ...
        if ! command -v unzip >/dev/null 2>&1; then
            fn_print_warning "正在安装 unzip 工具..."
            pkg install -y unzip >/dev/null 2>&1
        fi

        mkdir -p "$dest_dir"
        fn_print_warning "正在释放数据包至 $dest_dir ..."
        if unzip -q -o "$selected_file" -d "$dest_dir"; then
            fn_print_success "精准数据恢复完成！"
            echo -e "  ${GREEN}✔ 角色卡、聊天历史、插件和配置已就位。${NC}"
        else
            fn_print_error "ZIP 解压失败！请检查文件是否损坏。"
        fi
    else
        fn_print_error "不支持的备份压缩格式 (仅支持 .tar.gz / .zip)！"
    fi

    fn_press_any_key
}

fn_github_sync_and_autostart() {
    clear
    fn_print_header "从 GitHub 仓库拉取散文件并配置开机自启"
    echo -e "  ${YELLOW}说明：此功能支持从你的 GitHub 酒馆仓库直接拉取散文件，${NC}"
    echo -e "    自动补齐 Node 依赖，并可选配置【每次打开 Termux 直接自启动】！\n"

    echo -e "  请输入你的 GitHub 仓库地址 (支持完整 URL 或 用户名/仓库名):"
    echo -e "  示例: ${CYAN}https://github.com/yourname/my-sillytavern.git${NC} 或 ${CYAN}yourname/my-sillytavern${NC}"
    echo -n "  仓库地址: "
    read -r repo_input

    if [ -z "$repo_input" ]; then
        fn_print_warning "未输入仓库地址，操作已取消。"
        fn_press_any_key
        return
    fi

    local repo_url="$repo_input"
    if [[ "$repo_input" != http* ]] && [[ "$repo_input" == */* ]]; then
        repo_url="https://github.com/${repo_input}.git"
    fi

    local dest_dir="$ST_DIR"
    echo -e "\n  请输入本地存放的目标路径："
    echo -e "  [直接回车默认: ${CYAN}${dest_dir}${NC}]"
    echo -n "  目标路径: "
    read -r user_dest
    [ -n "$user_dest" ] && dest_dir="$user_dest"

    fn_print_warning "正在检查环境与网络线路..."
    if ! command -v git >/dev/null 2>&1; then
        fn_print_info "正在安装 git..."
        pkg install -y git >/dev/null 2>&1
    fi
    if ! command -v node >/dev/null 2>&1; then
        fn_print_info "正在安装 nodejs-lts..."
        pkg install -y nodejs-lts >/dev/null 2>&1
    fi

    local clone_url="$repo_url"
    if fn_read_yes_no_prompt "是否使用国内加速镜像拉取 GitHub 仓库？" true ""; then
        clone_url="https://ghfast.top/${repo_url}"
    fi

    if [ -d "$dest_dir/.git" ]; then
        fn_print_info "检测到目标目录已存在 Git 仓库，正在拉取最新代码与数据..."
        (
            cd "$dest_dir" || exit 1
            git pull
        )
    else
        fn_print_info "正在克隆远程仓库到 ${dest_dir} ..."
        mkdir -p "$(dirname "$dest_dir")"
        if ! git clone "$clone_url" "$dest_dir"; then
            fn_print_warning "镜像拉取失败，尝试官方直连..."
            git clone "$repo_url" "$dest_dir"
        fi
    fi

    if [ ! -d "$dest_dir" ]; then
        fn_print_error "拉取仓库失败，请检查仓库地址是否正确或是否为私有仓库！"
        fn_press_any_key
        return
    fi

    fn_print_info "正在配置国内 NPM 镜像并校验安装依赖..."
    (
        cd "$dest_dir" || exit 1
        npm config set registry https://registry.npmmirror.com
        npm install --no-audit --no-fund --omit=dev
    ...
    if fn_read_yes_no_prompt "是否配置【每次打开 Termux 自动启动酒馆】？" true ""; then
        local bashrc_file="$HOME/.bashrc"
        # 先清除旧的自动启动代码
        if [ -f "$bashrc_file" ]; then
            sed -i '/# GUGU_ST_AUTOSTART_BEGIN/,/# GUGU_ST_AUTOSTART_END/d' "$bashrc_file"
        fi

        cat << EOF >> "$bashrc_file"
# GUGU_ST_AUTOSTART_BEGIN
if [ -d "$dest_dir" ] && [ -f "$dest_dir/server.js" ]; then
    echo -e "\\033[32m[自启] 正在为您启动 SillyTavern...\\033[0m"
    cd "$dest_dir" && node server.js
fi
# GUGU_ST_AUTOSTART_END
EOF
        fn_print_success "已成功将开机自启命令写入 ~/.bashrc ！"
        echo -e "  ${YELLOW}提示：若以后想关闭自启，可在本菜单或手动编辑 ~/.bashrc 清理。${NC}"
    fi

    if fn_read_yes_no_prompt "是否现在立即启动酒馆？" true ""; then
        cd "$dest_dir" && node server.js
    fi
...
fn_menu_oneclick_backup() {
    while true; do
        clear
        fn_print_header "一键打包与数据恢复管理"
        echo -e "      [1] ${CYAN}Termux 全量环境与数据打包 (含环境/Node/酒馆)${NC}"
        echo -e "      [2] ${GREEN}酒馆资产轻量精准打包 (仅含角色/对话/插件/配置)${NC}"
        echo -e "      [3] ${YELLOW}解压与恢复备份包 (指定备份还原至目标目录)${NC}"
        echo -e "      [4] ${SOFT_GOLD}从 GitHub 仓库拉取散文件并配置开机自启${NC}"
        echo -e "      [0] ${CYAN}返回主菜单${NC}\n"

        local choice
        choice="$(fn_read_menu_prompt "0-4")"
        case "$choice" in
            1) fn_full_system_backup ;;
            2) fn_precision_tavern_backup ;;
            3) fn_restore_backup ;;
            4) fn_github_sync_and_autostart ;;
            0) return ;;
            *) fn_print_error "输入无效，请按提示重试。"; sleep 1 ;;
        esac
    done
...
while true; do
    ...
    printf "      "; fn_print_menu_cell "$SOFT_PEACH" 13 "首次安装Luker"; fn_print_menu_cell "$SOFT_AQUA" 14 "酒馆/Luker切换"; printf "\n"
    printf "      "; fn_print_menu_cell "$SOFT_ROSE" 15 "启动Luker"; fn_print_menu_cell "$SOFT_GOLD" 16 "一键打包/恢复"; printf "\n\n"
    choice="$(fn_read_menu_prompt "0-16")"
    ...
    case $choice in
        ...
        12) fn_menu_gugu_box ;;
        13) fn_install_luker ;;
        14) fn_switch_st_luker_mode ;;
        15) fn_start_luker ;;
        16) fn_menu_oneclick_backup ;;
