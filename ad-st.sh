...
fn_get_st_current_mode() {
    ...
    branch="$(git -C "$ST_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || true)"
    ...
    if [[ "$current_mode" == "luker" ]]; then
        ...
        case "$choice" in
            1)
                ...
                if ! git switch release 2>/dev/null; then
            ...
            2)
                ...
                git fetch luker 2>/dev/null || true
        ...
        case "$choice" in
            1)
                ...
                if ! git remote | grep -q "^luker$"; then
                    ...
                    git remote add luker "$luker_repo_url" 2>/dev/null || git remote set-url luker "$luker_repo_url"
                fi
                fn_print_warning "正在获取 Luker 远程分支信息 (git fetch luker)..."
                if ! fn_run_git_with_progress "获取 Luker 分支" false git fetch luker; then
                    fn_print_warning "直连 fetch 失败，尝试通过加速代理重试..."
                    git remote set-url luker "https://ghfast.top/https://github.com/funnycups/Luker.git" 2>/dev/null || true
                    fn_run_git_with_progress "获取 Luker 分支(代理)" false git fetch luker || fn_print_warning "fetch 异常，尝试直接切换..."
                fi

                fn_print_warning "正在切换至 luker 分支..."
                if git show-ref --verify --quiet refs/heads/luker; then
                    git switch luker || git checkout luker
                else
                    git switch -c luker --track luker/release 2>/dev/null || git checkout -b luker luker/release 2>/dev/null || git checkout -b luker
                fi
